...
Estat canònic del motor agentiu `3.3`, sobre el contracte de proposta `3.1`.
...
3. dona al model eines de lectura sobre el catàleg privat i les guies;
4. deixa que el model decideixi l’estratègia de cerca i la planificació;
5. comprova els identificadors, la compatibilitat, el temps i la integritat del contracte;
6. mostra en directe el progrés segur de l’anàlisi, les cerques i les validacions;
7. presenta la proposta, les personalitzacions i un resum auditable de la cerca;
8. no modifica la sessió fins que l’entrenador prem **Afegir a la sessió**.
...
decisió humana          participants actius
        ...
        └────── agent LLM amb function calling ──────┐
                                                     │
  search_exercises ←→ details ←→ guidance ←→ compatibility
          ↑                         ↓               ↓
          └──── alternatives compatibles ──── timing/audit
                                                     ↓
                              BlockGenerationProposal 3.1
                                                     ↓
                          validador canònic del servidor
                                                     ↓
                           revisor LLM independent
                         ↙ revisa            aprova ↘
                  reparació agentiva          proposta final
                                                     ↓
...
- detecció de dades clíniques estructurals desconegudes que necessiten una decisió;
...
modalitats, dificultat, estat editorial, compatibilitat de material i paginació. Els termes
...
consignes, seguretat, restriccions i procedència.
...
`monitor`, `requires_modification`, `incompatible` i `uncertain`; una condició sense regió
ja no es considera silenciosament aplicable a tot el cos.

### `find_compatible_alternatives`

Busca substitucions per a una gimnasta i un exercici base. Pot conservar el patró o
ampliar-lo, respecta el material i l’estat editorial, descarta incompatibilitats conegudes
i retorna els motius de rebuig. Els candidats vàlids s’incorporen a l’allowlist de la run.
...
- Cada condició `avoid` o `modify` necessita una decisió estructurada; les accions
  `modify`, `replace` o `skip` han de correspondre amb ajustaments als ítems indicats.
...
Cada condició té ara un abast explícit: `regional`, `global` o `unknown`. Una condició
`avoid` o `modify` amb abast desconegut no es converteix en una prohibició global: abans
de generar, la UI demana si l’agent pot buscar adaptacions prudents o si la gimnasta s’ha
d’excloure només d’aquell bloc. Des del perfil, **Corregir dades** crea una versió pendent
que substitueix la condició anterior quan és confirmada, mantenint l’historial.

La resta d’informació arriba a l’agent:

- `avoid`: ha de motivar una substitució o un `skip` individual;
- `modify`: pot modificar dosi, intensitat, descans o exercici;
- `monitor`: pot mantenir l’exercici amb seguiment explícit;
- edat desconeguda: és una incertesa que el model ha de gestionar prudentment, però ja no
  interromp automàticament tot el flux;
...
Els lectors de serialització continuen admetent els contractes `1.0`, `2.0` i `3.0` que ja
poguessin estar desats.
...
- `validation_payload`: intents de validació, errors, allowlist i últim càlcul temporal;
...
El revisor retorna `pass`, `revise` o `needs_clarification`:

- `pass`: la proposta es pot mostrar;
- `revise`: els errors concrets tornen al planificador dins del mateix límit de reparacions;
...
El veredicte, el resum, els intents i els IDs de resposta queden a `validation_payload`.
La UI mostra que hi ha hagut revisió independent i el seu resum final. Aquesta segona crida
augmenta cost i latència; s’ha d’incloure explícitament en els benchmarks.
...
- contracte `3.1` i validació del servidor;
...
independents poden agrupar-se en paral·lel. Cal tornar a mesurar latència i cost en els
...
1. completar les regions o els abasts desconeguts dels perfils que apareguin a la UI;
